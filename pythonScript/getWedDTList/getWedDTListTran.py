...
# 查询未翻译的数据
cursor.execute("""
    SELECT id, context, results, comment
    FROM src_wed_dt_playlist_import
    WHERE ai_tran_context IS NULL
       OR ai_tran_results IS NULL
       OR ai_tran_comment IS NULL
     -- limit 10
""")
...
for row in rows:
    ...
    tran_results = translate_text(results)
    tran_comment = translate_text(comment)

    # 打印进度
    print(f"✔ 正在翻译 ID={id}")
    print(f"  説: {context} ➡ {tran_context}")
    print(f"  検証: {results} ➡ {tran_results}")
    print(f"  コメント: {comment} ➡ {tran_comment}\n")

    # 写入数据库
    try:
        cursor.execute("""
            UPDATE src_wed_dt_playlist_import
            SET ai_tran_context=%s,
                ai_tran_results=%s,
                ai_tran_comment=%s
            WHERE id=%s
        """, (tran_context, tran_results, tran_comment, id))
        conn.commit()
    except Exception as e:
